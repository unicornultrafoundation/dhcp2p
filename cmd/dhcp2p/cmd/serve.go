...
func serveCmd() *cobra.Command {
	...
	cmd.Flags().StringP(flag.PASSWORD_FLAG, flag.PASSWORD_FLAG_SHORT, "", "Password/Path to the password file")
	cmd.Flags().StringP(flag.ACCOUNT_FLAG, flag.ACCOUNT_FLAG_SHORT, "", "Account address to use")
	...
	// Required flags
	cmd.MarkFlagRequired(flag.PASSWORD_FLAG)
	...
	viper.BindPFlag("password", cmd.Flags().Lookup(flag.PASSWORD_FLAG))
	viper.BindPFlag("account", cmd.Flags().Lookup(flag.ACCOUNT_FLAG))
